...
class TradeRecorder:
    ...
    def get_results_paths(self, config: Dict) -> Dict[str, str]:
        """Возвращает пути к папкам для сохранения результатов"""
    ...
    @staticmethod
    def export_optimization_summary(optimization_results: List[Dict],
                                    config: Dict,
                                    filename: str = None) -> str:
        """
        Экспортирует сводку результатов оптимизации в корневую папку results/
        ИСПРАВЛЕНИЕ: Убираем дублирование - теперь только сводка без детализации
        """
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        results_dir = "results"
        TradeRecorder._ensure_directory_exists(results_dir)
        filepath = os.path.join(results_dir, filename)

        try:
            with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
                # Лист с результатами оптимизации
                summary_df = TradeRecorder._prepare_optimization_summary_dataframe(optimization_results)
                summary_df.to_excel(writer, sheet_name='Результаты_Оптимизации', index=False)
                TradeRecorder._format_optimization_summary_sheet(writer.sheets['Результаты_Оптимизации'])

                # Лист со статистикой оптимизации
                stats_df = TradeRecorder._prepare_optimization_stats_dataframe(optimization_results, config)
                stats_df.to_excel(writer, sheet_name='Статистика_Оптимизации', index=False)
                TradeRecorder._format_stats_sheet(writer.sheets['Статистика_Оптимизации'])

            print(f"Сводка оптимизации сохранена: {filepath}")
            return filepath

        except Exception as e:
            print(f"Ошибка экспорта сводки оптимизации: {e}")
            return ""

    @staticmethod
    def _prepare_optimization_summary_dataframe(results: List[Dict]) -> pd.DataFrame:
        """Подготавливает DataFrame с результатами оптимизации"""
        summary_data = []
        for result in results:
            summary_data.append({
                'Левые бары': result.get('left_bars', 0),
                'Правые бары': result.get('right_bars', 0),
                'Доходность %': result.get('total_return', 0),
                'Общий PnL $': result.get('total_pnl', 0),
                'Макс просадка %': result.get('max_drawdown_percent', 0),
                'Коэфф. Шарпа': result.get('sharpe_ratio', 0),
                'Профит-фактор': result.get('profit_factor', 0),
                'Винрейт %': result.get('win_rate', 0),
                'Всего сделок': result.get('total_trades', 0),
                'Средняя сделка $': result.get('avg_trade', 0)
            })

        return pd.DataFrame(summary_data)

    @staticmethod
    def _prepare_optimization_stats_dataframe(results: List[Dict], config: Dict) -> pd.DataFrame:
        """Подготавливает DataFrame со статистикой оптимизации"""
        successful_results = [r for r in results if r.get('success', False)]
        profitable_results = [r for r in results if r.get('total_return', 0) > 0]

        # Получаем диапазоны из конфигурации
        optimization_config = config.get('optimization', {})
        left_range = optimization_config.get('left_bars_range', (0, 0))
        right_range = optimization_config.get('right_bars_range', (0, 0))

        stats_data = [
            ('Всего комбинаций', len(results)),
            ('Успешных результатов', len(successful_results)),
            ('Прибыльных комбинаций', len(profitable_results)),
            ('Диапазон левых баров', f"{left_range[0]}-{left_range[1]}"),
            ('Диапазон правых баров', f"{right_range[0]}-{right_range[1]}"),
            ('Метрика ранжирования', optimization_config.get('ranking_metric', 'total_return')),
            ('Максимум процессов', optimization_config.get('max_workers', 'Авто') or 'Авто'),
            ('Сохранять только прибыльные', optimization_config.get('save_only_profitable', True))
        ]

        return pd.DataFrame(stats_data, columns=['Параметр', 'Значение'])
